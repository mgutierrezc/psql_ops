    ...
    * primary_keys: list of variable names that should be treated as primary keys
    ...
    for column in dataframe:
        ...
        if reserved == False:
            ...
            # Add "_var" to reserved variable name to make it usable
            pair = f"\"{column.lower()}_var\" "
    ...
    if (primary_keys != None) and (len(primary_keys) != primary_key_count):
        raise ValueError(
            f"{len(primary_keys) - primary_key_count} of the primary keys given were not found in the dataframe or were reserved keywords")
    ...
    print(
        f"Variable names that were found to be keywords and subsequently modified by adding _var: {changed_varnames_str}\n\n")
...
if __name__ == '__main__':
    ...
    dataframe_path = input("Input your dataframe file path: ")

    if dataframe_path.endswith(".csv"):
        dataframe = pd.read_csv(dataframe_path)
    elif dataframe_path.endswith((".xlsx", ".xls")):
        dataframe = pd.read_excel(dataframe_path)

    pre_primary_keys = input("Enter primary keys (if multiple, separate them by a space): ")
